  ...
  onCenterChange(center: LatLng) {
	...
	readMaptilerSource (url:string) {
		...
		//this.http.get(url, { responseType: "text" }).subscribe(response => {
			...
			console.info("stripped: ", strippedData);
	...
	initLeafletMap (url:string, data:any) {
		...
		console.info("init leaflet ==> " + "CENTER: " + this.currentCenter + " ZOOM: " + this.currentZoom + " BOUNDS: " + this.currentBounds);
	...
	onMapReady(map: L.Map) {
		map.on(L.Draw.Event.CREATED, function (e) {
			...
			if (type === 'rectangle') {
				...
				// for(let i = 0; i < this.rectangleCoordinates.length; i++){
				// 	console.info()
				// }
